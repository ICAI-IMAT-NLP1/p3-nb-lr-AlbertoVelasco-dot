...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        class_priors: Dict[int, torch.Tensor] = {label: torch.tensor(counter[label]/len(labels))
    ...
    def estimate_conditional_probabilities(
        ...
        class_word_counts: Dict[int, torch.Tensor] = {
            label.item(): 
                (torch.ones(self.vocab_size)*delta + torch.sum(features[labels == label], dim=0)) /
                (torch.sum(features[labels == label]) + torch.tensor(delta*self.vocab_size))
            for label in labels
        }
    ...
    def estimate_class_posteriors(
        ...
        log_posteriors: torch.Tensor = torch.tensor([
            torch.log(class_prior) + torch.sum(feature * torch.log(self.conditional_probabilities[label])) 
            for label, class_prior in self.class_priors.items()])
    ...
    def predict(self, feature: torch.Tensor) -> int:
        ...
        pred: int = self.estimate_class_posteriors(feature).argmax().item()
        return pred
    ...
    def predict_proba(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        probs: torch.Tensor = self.estimate_class_posteriors(feature).exp()
